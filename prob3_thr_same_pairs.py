...
def _normalize_emb_tag(emb_tag: str) -> str:
	emb_tag = str(emb_tag)
	for prefix in ("evr_compare__", "fnmr_vs_reject__", "fmr_vs_reject__", "cer_vs_reject__"):
		if emb_tag.startswith(prefix):
			return emb_tag[len(prefix) :]
	return emb_tag


def _extract_emb_tag_from_detailed_dir(detailed_dir: Path) -> str:
	prefix = detailed_dir.name[: -len("_detailed")] if detailed_dir.name.endswith("_detailed") else detailed_dir.name
	return _normalize_emb_tag(prefix)


def _resolve_embeddings_pkl(fr_features_root: Path, dataset_name: str, fr_model: str, emb_tag: str) -> Path:
	emb_tag = _normalize_emb_tag(emb_tag)
...
def _plot_threshold_summary(
	...
	*,
	...
	fr_model = detailed_dir.parent.parent.name
	dataset_name = detailed_dir.parent.name
	emb_tag = _extract_emb_tag_from_detailed_dir(detailed_dir)
