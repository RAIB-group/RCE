...
EVD comparison plot across all FIQA methods found under:

    /home/bw/FIQA/fiq_baselines/quality_scores/<method>/{dataset_name}-quality.pkl

Outputs are written to:

    output/sol1_evd_fix_thr/{dataset_name}/{fr_model}/
"""
...
DEFAULT_OUT_ROOT = os.path.join("output", "sol1_evd_fix_thr")
...
def error_vs_reject(
    ...
    Args:
        ...
        threshold_method: "percentile" or "roc"
        cer_lambda: weight for CER = lambda * FMR + (1-lambda) * FNMR
    ...
    for reject_fraction in reject_steps:
        ...
        min_impostors_needed = int(np.ceil(1.0 / fmr_target))
...
def plot_evr_compare_dataset(
    *,
    ...
    if not cer_df.empty:
        cer_df["rank"] = cer_df["pAUC@0.3"].rank(method="min", ascending=True).astype(int)
...
if __name__ == "__main__":
    ...
    p.add_argument(
        ...
        help="Output root directory (default: output/evd_fix_thr)",
    )
    p.add_argument(
        "--run-all",
        action="store_true",
    ...
    run_prob1_test_set_batch(
        ...
        cer_lambda=float(args.cer_lambda),
    )
