...
To run from ranking CSVs (harm ranking mode):
    ...
    python edc_rank_quality.py \
        ...
        --out-root /home/bw/FIQA/evd_ijcb/output/evd_ranking \
        --threshold-method roc

    Note: when both --rank-root and --quality-dir are set, the script also writes a
    comparison EVR plot that includes harm ranking plus all FIQA quality methods.
...
Outputs are written to:
    ...
    output/evd_ranking/{fr_model}/{dataset_name}/
"""
...
def _progress(iterable, desc=None):
    if tqdm is None:
        return iterable
...
DEFAULT_OUT_ROOT = os.path.join("output", "evd_ranking")

# Default allowlists (used unless --run-all is passed)
DEFAULT_DATASET_ALLOWLIST = ["adience", "lfw", "agedb", "calfw", "cplfw", "xqlfw"]
...
HARM_RANKING_PLOT_COLOR = "#0b1f3a"
...
def _method_color(method_name: str) -> str:
    ...
    if method_name in DEFAULT_METHOD_ALLOWLIST:
        idx = DEFAULT_METHOD_ALLOWLIST.index(method_name)
...
def load_rank_quality_csv(path):
    """Load ranking CSV with columns [img, rank] and return DataFrame [image_id, score].

    Larger rank means lower quality, so we negate rank to make lower-quality images
    get smaller scores (rejected first).
    """
    df = pd.read_csv(path)
    if "img" not in df.columns or "rank" not in df.columns:
        raise ValueError(f"Ranking CSV missing required columns 'img' and 'rank': {path}")
    df = df[["img", "rank"]].copy()
    df["image_id"] = df["img"].map(_norm_image_id)
...
def plot_evr_compare_dataset(
    *,
    ...
    detailed_by_method: dict[str, list[dict[str, object]]] = {}
    ...
    methods_for_csv = sorted(available_methods)
    methods_for_plot = [m for m in methods_for_csv if m in allowed_methods]

    for method_name in sorted(set(methods_for_csv) | set(methods_for_plot)):
        _run(method_name, method_qpath[method_name])

    if len(methods_for_plot) == 0:
        raise FileNotFoundError(
            f"No quality PKLs found for plotting under allowlist for dataset '{dataset_name}' under: {quality_methods_dir}"
        )
    ...
    if not pauc_df.empty:
        pauc_df["rank"] = pauc_df["pAUC@0.3"].rank(method="min", ascending=True).astype(int)
        pauc_df = pauc_df.sort_values(["rank", "pAUC@0.3", "method"], ascending=[True, True, True])

    plt.figure(figsize=(12, 8))
    for m in methods_for_plot:
        rr, fn = curves[m]
        p = pauc_df[pauc_df["m_n"] == m]["pAUC@0.3"].values
        p_txt = "nan" if len(p) == 0 or np.isnan(p[0]) else f"{p[0]:.4f}"
        is_prop = m == "proposed"
        lw = PLOT_LINE_WIDTHS["highlight"] if is_prop else PLOT_LINE_WIDTHS["summary"]
        if m == "harm_ranking":
            color = HARM_RANKING_PLOT_COLOR
        elif is_prop:
            color = "#111111"
        else:
        ...
        plt.plot(
            rr,
            fn,
            label=f"{_display_method_name(m)}",
            color=color,
            linewidth=lw,
...
def plot_evr_compare_dataset_with_extra(
    *,
    dataset_name: str,
    embeddings_pkl: str,
    quality_methods_dir: str,
    pairs_csv: str,
    out_path: str,
    extra_methods: dict[str, pd.DataFrame],
    fmr_target: float = 1e-3,
    reject_steps=None,
    threshold_method: str = "percentile",
):
    ...
    def _unique_name(name: str) -> str:
        ...
        base = name
        idx = 2
        while f"{base}_{idx}" in curves:
            idx += 1
        return f"{base}_{idx}"

    def _run_df(method_name: str, quality_df: pd.DataFrame) -> None:
        safe_name = _unique_name(method_name)
        rr, fn, detailed_results, _, _ = error_vs_reject(
            embeddings,
            pairs_df,
            quality_df,
        ...
        p = compute_pauc(rr, fn, max_reject=0.3)
        curves[safe_name] = (rr, fn)
        method_to_pauc[safe_name] = p
        methods.append(safe_name)
        detailed_by_method[safe_name] = detailed_results
    ...
    for extra_name, extra_df in extra_methods.items():
        _run_df(extra_name, extra_df)
    ...
    methods_for_csv = list(methods)
    methods_for_plot = [m for m in methods_for_csv if m in allowed_methods]
    if len(methods_for_plot) == 0:
        raise FileNotFoundError(
            f"No methods matched plotting allowlist for dataset '{dataset_name}' under: {quality_methods_dir}"
        )
    ...
    for m in methods_for_plot:
        ...
        p = pauc_df[pauc_df["m_n"] == m]["pAUC@0.3"].values
        p_txt = "nan" if len(p) == 0 or np.isnan(p[0]) else f"{p[0]:.4f}"
        is_highlight = m in {"proposed", "harm_ranking"}
        lw = PLOT_LINE_WIDTHS["highlight"] if is_highlight else PLOT_LINE_WIDTHS["summary"]
        if m == "harm_ranking":
            color = HARM_RANKING_PLOT_COLOR
        elif is_highlight:
            color = "#111111"
...
def plot_evr_single_quality(
    *,
    ...
    p = compute_pauc(rr, fn, max_reject=0.3)
    p_txt = "nan" if np.isnan(p) else f"{p:.4f}"

    plt.figure(figsize=(12, 8))
    line_color = HARM_RANKING_PLOT_COLOR if str(label) == "harm_ranking" else "#111111"
    plt.plot(
        rr,
        fn,
        label=f"{_display_method_name(label)}",
        color=line_color,
        linewidth=PLOT_LINE_WIDTHS["highlight"],
        alpha=PLOT_ALPHAS["per_method"],
    )
    plt.xlabel("Discard rate (%)", fontsize=PLOT_FONT_SIZES["labels"])
    plt.ylabel("FNMR", fontsize=PLOT_FONT_SIZES["labels"])
    ...
    detailed_csv = os.path.splitext(out_path)[0] + "_detailed.csv"
...
def _dir_has_harm_ranking_csv(root_dir: str) -> bool:
    """Return True if root_dir contains any harm_ranking__*.csv file recursively."""
    for cur_dir, _, files in os.walk(root_dir):
        for f in files:
            if f.startswith("harm_ranking__") and f.endswith(".csv"):
                return True
    return False


def _resolve_rank_root(rank_root: str) -> str:
    """Resolve ranking root, allowing small naming mismatches in sibling folders.

    Example handled automatically: "sol3_rank_sample" -> "sol4_rank_sample"
    when only one matching sibling contains harm ranking CSV files.
    """
    if os.path.isdir(rank_root):
        return rank_root

    parent = os.path.dirname(rank_root)
    base = os.path.basename(rank_root)
    if not os.path.isdir(parent):
        raise FileNotFoundError(f"rank_root not found: {rank_root}")

    # Prefer siblings sharing the same suffix, e.g., *_rank_sample.
    ...
    candidates = [
        ...
        and os.path.basename(d).endswith(suffix)
        and _dir_has_harm_ranking_csv(d)
    ]

    if len(candidates) == 1:
        resolved = candidates[0]
        print(f"[warn] rank_root not found: {rank_root}")
        print(f"[warn] Using detected ranking root: {resolved}")
        return resolved

    if len(candidates) > 1:
        cand_txt = ", ".join(candidates)
        raise FileNotFoundError(
            f"rank_root not found: {rank_root}. Multiple candidate ranking roots found: {cand_txt}"
        )
...
def run_prob2_test_set_batch(
    *,
    ...
    for dataset_dir in _progress(datasets, desc="Datasets"):
        ...
        for fr_model, emb_tag, emb_pkl in _progress(jobs, desc=f"{dataset_name} models"):
            out_dir = os.path.join(out_root, fr_model, dataset_name)
            out_path = os.path.join(out_dir, f"evr_compare__{emb_tag}.png")
...
def run_evd_from_rankings(
    *,
    ...
    """Compute EVR curves from harm_ranking__<fr_model>.csv under rank_root.

    Expected layout:
        rank_root/{dataset_name}/{fr_model}/harm_ranking__{fr_model}.csv
    If quality_dir is provided, also generate a comparison plot with FIQA methods.
    """
    if reject_steps is None:
        reject_steps = np.arange(0, 1, 0.05)
    ...
    fr_features_root = str(fr_features_root)
    ca_fiqa_data_root = str(ca_fiqa_data_root)
    rank_root = _resolve_rank_root(rank_root)
    out_root = str(out_root) if out_root is not None else rank_root

    if not os.path.isdir(fr_features_root):
        raise FileNotFoundError(f"fr_features_root not found: {fr_features_root}")

    datasets = [
        os.path.join(rank_root, d)
        for d in sorted(os.listdir(rank_root))
        if os.path.isdir(os.path.join(rank_root, d)) and not d.startswith(".")
    ]

    if len(datasets) == 0:
        raise FileNotFoundError(f"No dataset directories under: {rank_root}")

    print(f"Mode: rankings under {rank_root}")
    print(f"Found {len(datasets)} datasets under: {rank_root}")
    ...
    for dataset_dir in _progress(datasets, desc="Datasets"):
        ...
        if len(model_dirs) == 0:
            print(f"[skip] No FR model directories in: {dataset_dir}")
            continue

        for md in _progress(model_dirs, desc=f"{dataset_name} models"):
            fr_model = os.path.basename(md)
            rank_csv = os.path.join(md, f"harm_ranking__{fr_model}.csv")
            if not os.path.exists(rank_csv):
                print(f"[skip] Missing ranking: {rank_csv}")
                continue

            emb_dir = os.path.join(fr_features_root, dataset_name, fr_model)
            emb_pkls = _list_embeddings_pkls(emb_dir)
            if len(emb_pkls) == 0:
                # Fallback to dataset root if no per-model subdir exists
                emb_dir = os.path.join(fr_features_root, dataset_name)
                emb_pkls = _list_embeddings_pkls(emb_dir)
            if len(emb_pkls) == 0:
                print(f"[skip] No embeddings found for {dataset_name}/{fr_model}")
                continue

            quality_df = load_rank_quality_csv(rank_csv)

            for emb_pkl in _progress(emb_pkls, desc=f"{dataset_name}/{fr_model} embeddings"):
                emb_tag = _derive_embedding_name(emb_pkl)
                out_dir = os.path.join(out_root, fr_model, dataset_name)
                os.makedirs(out_dir, exist_ok=True)
                out_path = os.path.join(out_dir, f"evr_rank__{emb_tag}.png")
                print(f"[run] {dataset_name}/{fr_model}/{emb_tag} -> {out_path}")
                try:
                    plot_evr_single_quality(
                        dataset_name=dataset_name,
                        fr_model=fr_model,
                        embeddings_pkl=emb_pkl,
                ...
                if quality_dir is not None and os.path.isdir(quality_dir):
                    compare_path = os.path.join(out_dir, f"evr_compare__{emb_tag}.png")
                    print(
                        f"[run] {dataset_name}/{fr_model}/{emb_tag} (compare) -> {compare_path}"
                    )
                    try:
                        plot_evr_compare_dataset_with_extra(
                            dataset_name=dataset_name,
                            embeddings_pkl=emb_pkl,
                            quality_methods_dir=quality_dir,
                            pairs_csv=pairs_csv,
                            out_path=compare_path,
                            extra_methods={"harm_ranking": quality_df},
                            fmr_target=fmr_target,
                            reject_steps=reject_steps,
                            threshold_method=threshold_method,
                        )
                        ...
                        print(
                            f"[fail] {dataset_name}/{fr_model}/{emb_tag} (compare): {e}"
                        )
...
if __name__ == "__main__":
    ...
    p.add_argument(
        ...
        help="Output root directory (default: output/evd_ranking)",
    )
    p.add_argument(
        "--run-all",
        action="store_true",
    ...
    p.add_argument(
        ...
        default=None,
        help=(
            "Root folder containing harm_ranking__<fr_model>.csv to compute EVR from rankings. "
            "If set, runs ranking-based EVR and can also compare with quality-dir."
        ),
    )
    args = p.parse_args()

    if args.rank_root:
        run_evd_from_rankings(
