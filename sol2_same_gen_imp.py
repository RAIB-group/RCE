...
Outputs are written to:
    ...
    output/sol2_same_gen_imp/{dataset_name}/{fr_model}/
"""
...
DEFAULT_OUT_ROOT = os.path.join("output", "sol2_same_gen_imp")
...
def error_vs_reject(embeddings, pairs_df, quality_df, id_to_idx,
    ...
    for reject_fraction in reject_steps:
        ...
        if target_reject_total <= 0:
            ...
            # If one class hits its limit, reassign remaining budget to the other.
    ...
    for dataset_dir in datasets:
        ...
        for fr_model, emb_tag, emb_pkl in jobs:
            ...
            for method_scope_dir, scope_run_all in scopes:
                ...
                try:
                    pauc_df = plot_evr_compare_dataset(
                    ...
                    if isinstance(pauc_df, pd.DataFrame) and (not pauc_df.empty):
...
if __name__ == "__main__":
    ...
    p.add_argument(
        ...
        help="Output root directory (default: output/sol2_same_gen_imp)",
    )
    p.add_argument(
        "--run-all",
        action="store_true",
